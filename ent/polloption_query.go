...
func (poq *PollOptionQuery) QueryPoll() *PollQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2M, true, polloption.PollTable, polloption.PollPrimaryKey...),
		)
...
//		GroupBy(polloption.FieldOption).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
func (poq *PollOptionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*PollOption, error) {
	...
	if query := poq.withPoll; query != nil {
		if err := poq.loadPoll(ctx, query, nodes,
			func(n *PollOption) { n.Edges.Poll = []*Poll{} },
			func(n *PollOption, e *Poll) { n.Edges.Poll = append(n.Edges.Poll, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (poq *PollOptionQuery) loadPoll(ctx context.Context, query *PollQuery, nodes []*PollOption, init func(*PollOption), assign func(*PollOption, *Poll)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(polloption.PollTable)
		s.Join(joinT).On(s.C(poll.FieldID), joinT.C(polloption.PollPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(polloption.PollPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(polloption.PollPrimaryKey[1]))
	...
	neighbors, err := withInterceptors[[]*Poll](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "poll" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
